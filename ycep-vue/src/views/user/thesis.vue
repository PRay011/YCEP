      ...
      <div class="content">
        ...
        >
          ...
          <div class="user-input">
            ...
            />
            <p class="search">
              <el-icon size="25"><Promotion @click="sendMessage()" /></el-icon>
            </p>
          </div>
        </div>
      </div>
  ...
  methods: {
      ...
      this.introOption.steps = [
        {
          ...
          position: "right",
          intro:
            "鼠标悬浮在各个功能模块上，可快速查找系统对应操作SOP以及运维人员。",
        },
        {
          title: "个人信息",
          element: "#step2",
          position: "right",
          intro:
            "点击个人头像/下拉图标，选择个人信息，初始密码为******，建议修改为个人账号密码。",
        },
        {
          title: "重新引导",
          element: "#step3",
    ...
    startResize(event: any) {
      ...
      if (isBorderClicked) {
        ...
        document.addEventListener("mouseup", this.stopResizeContainer);
      }
    },
    resizeContainer(event: any) {
      if (this.isResizing) {
        ...
        this.containerLeft = this.initialX - event.clientX;
      }
    },
    stopResizeContainer() {
      this.isResizing = false;
      document.removeEventListener("mousemove", this.resizeContainer);
      document.removeEventListener("mouseup", this.stopResizeContainer);
    },
  },
